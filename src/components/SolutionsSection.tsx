...
const solutions = [
  {
    ...
    en: 'AI 지능형 기록물 통합 관리 플랫폼',
    desc: '스캔 문서의 OCR 인식부터 AI 자동 요약, 지능형 검색 및 열람 승인 워크플로우를 하나로 통합한 코이션의 차세대 플래그십 솔루션입니다.',
    features: [
      '고정밀 멀티모달 OCR & 내용 검색',
      'LLM 기반 방대 기록물 핵심 요약',
      'RFID/QR 자동화 문서 등록 및 추적',
      '비대면 열람 신청 및 통합 승인 절차',
      '다운로드 없는 실시간 전체 미리보기',
      '메모/편집 및 행정박물 통계 관리',
    ],
    ctas: [{ label: '솔루션 상세 보기', primary: true }, { label: '실시간 데모 체험', primary: false }],
    featured: true,
  },
  {
    name: 'Zero Track!',
    badge: null,
    en: '비전자기록물 실시간 관리 시스템',
    desc: 'RFID 및 모바일 기술을 통해 서고 내 비전자기록물의 반입, 반출, 위치추적을 실시간으로 관리합니다. 기록물 분실 Zero화를 실현합니다.',
    features: [
      'RFID 기반 실시간 재고조사',
      '모바일 단말기 현장 연동',
      '기록물 무단 반출 차단 시스템',
    ...
    ctas: [{ label: '솔루션 상세 보기', primary: true }],
    featured: false,
  },
  {
    name: 'AI Masking',
    badge: 'Demo Available',
    en: 'AI 개인정보 자동 검증/마스킹',
    desc: '기록물 및 공문서 내 성명, 주민번호, 주소 등 민감한 개인정보를 AI가 자동으로 탐지하여 마스킹 처리합니다.',
    features: [
      '딥러닝 기반 텍스트/영역 탐지',
      '주민등록번호/지문 자동 식별',
      '대량 문서 고속 일괄 처리',
    ...
    ctas: [{ label: '실시간 데모 체험', primary: true }],
    featured: false,
  },
  {
    name: 'Vision Search',
    badge: 'Demo Available',
    en: 'AI 유사 이미지 검색 엔진',
    desc: '수십만 건의 이미지 데이터 중 사용자가 찾는 이미지와 시각적으로 유사한 데이터를 벡터 임베딩 기술로 즉시 찾아냅니다.',
    features: [
      '특징점 기반 벡터 검색 엔진',
      '고문헌/유물 이미지 매칭',
      '유사 패턴 자동 분류 시스템',
        ...
        <FadeUp delay={0.05} style={{ marginBottom: '20px' }}>
          <div style={{
            ...
            <div>
              ...
              <h3 style={{ fontSize: 'clamp(24px, 3vw, 36px)', fontWeight: 700, color: '#fff', marginBottom: '8px' }}>
                {featured.name}
              </h3>
              <p style={{ fontSize: '14px', color: '#FF0000', fontWeight: 600, marginBottom: '20px', letterSpacing: '0.04em' }}>
                {featured.en}
              </p>
              <p style={{ fontSize: '15px', color: 'rgba(255,255,255,0.6)', lineHeight: 1.75, marginBottom: '32px' }}>
                {featured.desc}
              </p>
              <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
                {featured.ctas.map((cta, i) => (
                  <a key={i} href="#contact" style={{
                    ...
                    background: cta.primary ? '#FF0000' : 'transparent',
                    color: '#fff',
                    border: cta.primary ? 'none' : '1px solid rgba(255,255,255,0.3)',
                  }}>
                    {cta.label}
                  </a>
                ))}
            ...
            {/* Right — feature list */}
            <div>
              <div style={{ fontSize: '12px', color: 'rgba(255,255,255,0.35)', letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: '20px' }}>
                Key Features
              </div>
          ...
          {rest.map((sol, i) => (
            <FadeUp key={i} delay={0.1 + i * 0.08}>
              <div style={{
                ...
                <div style={{ marginBottom: '16px' }}>
                  {sol.badge ? (
                    <span style={{
                      ...
                      border: '1px solid rgba(34,197,94,0.35)', borderRadius: '100px',
                    }}>
                      {sol.badge}
                    </span>
                  ) : (
                ...
                <p style={{ fontSize: '12px', color: '#FF0000', fontWeight: 600, marginBottom: '14px', letterSpacing: '0.04em' }}>{sol.en}</p>
                <p style={{ fontSize: '13px', color: 'rgba(255,255,255,0.5)', lineHeight: 1.7, marginBottom: '24px', flex: 1 }}>{sol.desc}</p>
